...
module.exports = function(options) {
	...
	function collectSassDeps(file, enc, next) {
		...
		// sending it on its way.
			...
			return next(new PluginError('gulp-sass-deps',
	...
	function writeSassDeps(done) {
		...
		if (!entryFile.isBuffer()) {
			return done(new PluginError('gulp-sass-deps',
		...
		contents.replace(importBlockRegex, '').match(existingImportRegex)
		...
		// remove paths that have already been manually imported.
		.filter(function(importPath) {
			return !~existingImports.indexOf(importPath);
		}).sort(function(a, b) {
			return a > b;
		});
